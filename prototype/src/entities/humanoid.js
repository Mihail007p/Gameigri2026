...
export class Rig {
  ...
  /* ═══════════ рыцарь (игрок): точное следование референсу ═══════════
     docs/art/knight_reference.png: скруглённый шлем с золотым крестом на
     личине, пышный меховой воротник, трёхпластинчатые наплечники с золотой
     кромкой, V-образный багровый табард с брошами и заклёпанной кирасой,
     стальной клёпаный пояс, длинная юбка и тассеты, расширяющийся плащ,
     круглый деревянный щит со стальным умбоном и бледным драконом, полуторный
     меч со стальной гардой. По-прежнему 5 слитых мешей — draw calls не растут. */
  _buildKnight(o) {
    const c = o.colors;
    const S = Math.PI / 2;

    /* ноги: стальные поножи, коленный налобник, коричневые сапоги с отворотом */
    const legParts = () => [
      place(box(0.22, 0.78, 0.24, c.pants), 0, -0.43, 0),
      place(cyl(0.10, 0.11, 0.12, 6, c.metal), 0, -0.44, 0.13, S),
      place(box(0.25, 0.10, 0.27, c.boots), 0, -0.66, 0.01),   // отворот сапога
      place(box(0.23, 0.24, 0.25, c.boots), 0, -0.80, 0.02),
      place(box(0.24, 0.12, 0.34, c.boots), 0, -0.90, 0.05),   // ступня
    ];
    this._part('legL', merge(legParts()), -0.145, HIP, 0);
    this._part('legR', merge(legParts()), 0.145, HIP, 0);

    const bp = [
      /* кираса с рядом заклёпок по центру */
      place(box(0.50, 0.60, 0.30, c.tunic), 0, 0.34, 0),
      place(box(0.035, 0.035, 0.02, c.metal), 0, 0.44, 0.158),
      place(box(0.035, 0.035, 0.02, c.metal), 0, 0.34, 0.158),
      place(box(0.035, 0.035, 0.02, c.metal), 0, 0.24, 0.158),
      /* V-образный багровый табард с золотой кромкой поверх кирасы */
      place(box(0.15, 0.70, 0.05, c.tabard), -0.13, 0.34, 0.163, 0, 0, 0.42),
      place(box(0.15, 0.70, 0.05, c.tabard), 0.13, 0.34, 0.163, 0, 0, -0.42),
      place(box(0.032, 0.70, 0.055, c.trim), -0.185, 0.34, 0.160, 0, 0, 0.42),
      place(box(0.032, 0.70, 0.055, c.trim), 0.185, 0.34, 0.160, 0, 0, -0.42),
      /* круглые золотые броши-фибулы у ключиц */
      place(cyl(0.045, 0.045, 0.035, 8, c.trim), -0.17, 0.575, 0.235, S),
      place(cyl(0.045, 0.045, 0.035, 8, c.trim), 0.17, 0.575, 0.235, S),
      /* стальной клёпаный пояс + кожаный ремешок ниже */
      place(box(0.55, 0.13, 0.34, c.metal), 0, 0.05, 0),
      place(box(0.09, 0.09, 0.03, c.trim), 0, 0.05, 0.175),
      place(box(0.03, 0.03, 0.02, c.belt), -0.16, 0.05, 0.172),
      place(box(0.03, 0.03, 0.02, c.belt), 0.16, 0.05, 0.172),
      place(box(0.56, 0.07, 0.35, c.belt), 0, -0.045, 0),
      /* длинная багровая юбка (сужается к поясу) + стальные тассеты по бокам */
      place(cyl(0.17, 0.24, 0.56, 4, c.tabard).scale(1.15, 1, 0.5), 0, -0.32, 0.12, 0, Math.PI / 4, 0),
      place(box(0.15, 0.36, 0.07, c.shoulder), -0.21, -0.20, 0.09, 0, 0, 0.12),
      place(box(0.15, 0.36, 0.07, c.shoulder), 0.21, -0.20, 0.09, 0, 0, -0.12),
      /* плащ: расширяется книзу, до середины икры */
      place(cyl(0.20, 0.36, 1.05, 4, c.cloak).scale(1, 1, 0.30), 0, 0.12, -0.21, 0, Math.PI / 4, 0),
      /* пышный меховой воротник: пелерина + наплечные кули + нагрудник */
      place(box(0.62, 0.17, 0.40, c.fur), 0, 0.675, -0.02),
      place(box(0.40, 0.14, 0.12, c.fur), 0, 0.645, 0.16),
      place(box(0.17, 0.13, 0.26, c.fur), -0.27, 0.65, 0.02),
      place(box(0.17, 0.13, 0.26, c.fur), 0.27, 0.65, 0.02),
      /* наплечники: три пластины, каждая с золотой кромкой */
      place(box(0.26, 0.15, 0.32, c.shoulder), -0.36, 0.615, 0),
      place(box(0.27, 0.032, 0.33, c.trim), -0.36, 0.535, 0),
      place(box(0.23, 0.13, 0.29, c.shoulder), -0.41, 0.462, 0),
      place(box(0.24, 0.030, 0.30, c.trim), -0.41, 0.392, 0),
      place(box(0.19, 0.11, 0.25, c.shoulder), -0.44, 0.330, 0),
      place(box(0.20, 0.028, 0.26, c.trim), -0.44, 0.272, 0),
      place(box(0.26, 0.15, 0.32, c.shoulder), 0.36, 0.615, 0),
      place(box(0.27, 0.032, 0.33, c.trim), 0.36, 0.535, 0),
      place(box(0.23, 0.13, 0.29, c.shoulder), 0.41, 0.462, 0),
      place(box(0.24, 0.030, 0.30, c.trim), 0.41, 0.392, 0),
      place(box(0.19, 0.11, 0.25, c.shoulder), 0.44, 0.330, 0),
      place(box(0.20, 0.028, 0.26, c.trim), 0.44, 0.272, 0),
      /* шея и скруглённый шлем: золотой ободок, крест личины, прорези, гребень */
      place(box(0.14, 0.08, 0.14, c.belt), 0, 0.735, 0),
      place(cyl(0.148, 0.163, 0.24, 8, c.helm), 0, 0.88, 0),
      place(cone(0.146, 0.085, 8, c.helm), 0, 1.042, 0),
      place(cyl(0.166, 0.166, 0.045, 8, c.trim), 0, 0.955, 0),   // золотой ободок
      place(box(0.05, 0.21, 0.05, c.trim), 0, 0.855, 0.148),     // золотой наносник
      place(box(0.075, 0.042, 0.03, c.eyes), -0.078, 0.905, 0.148),
      place(box(0.075, 0.042, 0.03, c.eyes), 0.078, 0.905, 0.148),
      place(cyl(0.13, 0.155, 0.10, 8, c.helm), 0, 0.735, 0),     // подбородник
      place(box(0.035, 0.07, 0.24, c.crest), 0, 1.075, -0.02),   // низкий гребень
      place(box(0.05, 0.06, 0.06, c.crest), 0, 1.09, 0.10),      // перо спереди
    ];
    this._part('body', merge(bp), 0, HIP, 0);

    /* руки: стальное плечо, коричневый отворот и наруч, кожаная перчатка */
    const armParts = () => [
      place(box(0.16, 0.30, 0.18, c.shoulder), 0, -0.16, 0),
      place(box(0.175, 0.09, 0.19, c.boots), 0, -0.345, 0),
      place(box(0.15, 0.26, 0.16, c.belt), 0, -0.50, 0),
      place(box(0.12, 0.13, 0.13, c.boots), 0, -0.68, 0),
    ];

    /* левая рука + круглый деревянный щит: стальной обод, умбон, дракон */
    const la = armParts();
    if (o.shield) {
      la.push(place(cyl(0.325, 0.325, 0.045, 12, c.metal), 0.02, -0.46, 0.20, S));
      la.push(place(cyl(0.30, 0.30, 0.06, 12, c.shield), 0.02, -0.46, 0.22, S));
      la.push(place(cyl(0.085, 0.085, 0.10, 10, c.metal), 0.02, -0.46, 0.26, S));
      const e = c.emblem ?? c.fur;
      la.push(place(box(0.04, 0.08, 0.02, e), 0.02, -0.33, 0.252));          // шея
      la.push(place(box(0.08, 0.05, 0.02, e), 0.045, -0.28, 0.252, 0, 0, -0.25)); // голова
      la.push(place(box(0.05, 0.15, 0.02, e), 0.02, -0.55, 0.252));          // туловище
      la.push(place(box(0.03, 0.10, 0.02, e), 0.05, -0.64, 0.252, 0, 0, 0.5));  // хвост
      la.push(place(box(0.20, 0.055, 0.02, e), -0.12, -0.38, 0.252, 0, 0, 0.35)); // крыло левое
      la.push(place(box(0.20, 0.055, 0.02, e), 0.16, -0.38, 0.252, 0, 0, -0.35)); // крыло правое
    }
    this._part('armL', merge(la), -0.345, HIP + SHOULDER, 0);

    /* правая рука + полуторный меч: стальное навершие и гарда, кожаная рукоять */
    const ra = armParts();
    if (o.weapon !== false) {
      const w = o.weaponColors || c;
      ra.push(place(cyl(0.045, 0.045, 0.06, 8, w.metal), 0, -0.72, 0.02));
      ra.push(place(box(0.045, 0.20, 0.045, w.grip), 0, -0.83, 0.02));
      ra.push(place(box(0.30, 0.045, 0.08, w.metal), 0, -0.95, 0.02));
      ra.push(place(box(0.065, 0.76, 0.13, w.blade), 0, -1.35, 0.02));
      ra.push(place(cone(0.046, 0.14, 4, w.blade), 0, -1.79, 0.02, Math.PI));
    }
    const armR = this._part('armR', merge(ra), 0.345, HIP + SHOULDER, 0);
    this.tip = new THREE.Object3D();
    this.tip.position.set(0, -1.80, 0.02);
    armR.add(this.tip);
...
export const SKINS = {
  ...
  knight: {
    knight: true,
    skin: 0xd8a882, tunic: 0x8ba0ae, shoulder: 0x7b909f, belt: 0x4a3a2a,
    pants: 0x74899a, boots: 0x5a4632, hair: 0x849aa8, cloak: 0x872639,
    eyes: 0x14181c, metal: 0xaebdc7, blade: 0xcfd8de, grip: 0x3a2a1c,
    shield: 0x6b5138, helm: 0x849aa8, crest: 0xd2b06a, trim: 0xc9a860,
    tabard: 0x962c42, fur: 0xb7ab97, emblem: 0xcbb9a0, beard: 0x0,
  },
  nord: {
